...
export default function UpdateInstrument({ targetId }) {
  ...
  const targetInstrument =
    instruments.length > 0 ? instruments.find((item) => item.id === targetId) : instrument;
  ...
  useEffect(() => {
    ...
    if (submitted) {
      ...
      dispatch(instrumentUpdate([targetId, data])).then((res) => {
